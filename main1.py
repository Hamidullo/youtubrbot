...
def get_format(update : Update , context : CallbackContext ):
...
def download_choosen_format(update, context):
    ...
    with video.send() as files:
        #context.bot.send_document(chat_id=query.message.chat_id, document=open(files, 'rb'))
        for f in files:
            context.bot.send_document(chat_id=query.message.chat_id, document=open(f, 'rb'))

updater = Updater(token="1222893361:AAHNQvts9OU9tYRkrYOkGplmP9NsLOP1frA", use_context = True)
